...
# TODO GET /api/v1/users/:user_id/logins in user class
  ...
  def save
    response = Shuttle::Login.create(2, self)
    if response[0] == '200'
      return true
    else
      return false
  ...
  def update
    response = Shuttle::Login.edit(2, self)
    if response[0] == '200'
      return true
    else
      return false
  ...
  def self.get_all_by_user_id(user_id)
    ...
    if response[0] == '200'
       response[1].each do |response_part|
         login = Shuttle::Login.new(response_part)
         login.saved = true
         logins << login
       end
    end
  ...
  def self.get_all_by_sis_user_id(sis_user_id)
    ...
    if response[0] == '200'
       response[1].each do |login|
         logins << Shuttle::Login.new(login)
       end
    end
    return logins
  end
  ...
  def self.get_or_create_by_user_id_and_unique_id(user_id, unique_id)
    logins = Shuttle::Login.get_all_by_user_id(user_id)
    login = logins.find {|login| login.unique_id == unique_id }
    if login != nil
      login.saved = true
      return login
    else
      login = Shuttle::Login.new
      login.saved = false
  ...
  def self.get_or_create_by_sis_user_id_and_unique_id(sis_user_id, unique_id)
    logins = Shuttle::Login.get_all_by_sis_user_id(sis_user_id)
    login = logins.find {|login| login.unique_id == unique_id }
    if login != nil
      login.saved = true
      return login
